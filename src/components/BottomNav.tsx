...
const BottomNav = ({ activeTab, onTabChange }: BottomNavProps) => {
  ...
  return (
    <nav className={cn(
      'flex border-t border-border/50 pb-[env(safe-area-inset-bottom,0px)] flex-shrink-0 z-50 relative',
      resolvedTier === 'full' ? 'bg-background/94 backdrop-blur-[20px]' : 'bg-[#F8F6F1]/95'
    )}>
      {tabs.map(tab => (
        <button
          key={tab.id}
