    ...
    <div className="fixed inset-0 flex flex-col z-[1] overflow-hidden">
      ...
      <div
        ...
        {activeTab === 'program' &&
          <div className="px-3.5 pb-24">
            <div className="pt-6 pb-5 text-center border-solid border-primary">
              ...
              <h1 className="font-display text-[28px] font-bold leading-tight mb-2 bg-gradient-to-br from-gold-light via-gold to-destructive bg-clip-text text-transparent bg-mint">ЗРЕНИЕ В ФОКУСЕ</h1>
              <p className="leading-relaxed mb-5 max-w-[290px] mx-auto text-sm font-mono font-medium text-primary">комплекс упражнений для глаз<br />начните свой путь к ясному взгляду<br />маленькими шагами к лёгкости</p>
              ...
              <p className="text-[11px] text-muted-foreground tracking-wide text-center">{TOTAL} упражнений</p>
            </div>

            {sections.map((section, si) =>
              <div key={si}>
                <div className="flex items-center gap-3 mt-7 mb-3.5 opacity-85">
                  <span className="text-[9px] tracking-[3.5px] uppercase text-[hsl(var(--gold))] font-semibold whitespace-nowrap">{section.label}</span>
            ...
            {completedCount === TOTAL &&
              <div className="text-center py-9 px-6 bg-card border border-border rounded-[20px] mt-5">
                ...
                <h2 className="font-display text-2xl mb-2 bg-gradient-to-br from-gold-light to-gold bg-clip-text text-transparent">Ритуал выполнен!</h2>
                <p className="text-muted-foreground text-[13px] leading-relaxed">Отличная работа! Каждый день — это вклад<br />в здоровье глаз и шеи.</p>
              </div>
            }
          </div>
        }
        ...
        {activeTab === 'schedule' &&
          <div className="px-3.5 pb-24">
            <div className="pt-7 pb-5 text-center">
              <span className="inline-block text-[9px] tracking-[3.5px] uppercase text-[hsl(var(--gold))] border border-[hsl(var(--gold)/0.28)] px-4 py-1 rounded-full mb-3.5 font-medium">Распорядок дня</span>
              <h2 className="font-display text-[28px] font-bold bg-gradient-to-br from-gold-light to-gold bg-clip-text text-transparent">Расписание</h2>
            </div>
            {schedule.map((block, i) =>
              <div key={i} className="bg-card border border-border rounded-[20px] p-5 mb-3">
                <div className="text-[9px] tracking-[3px] uppercase text-[hsl(var(--gold))] font-semibold mb-2">{block.time}</div>
                ...
                <ul className="list-none text-[13px] text-muted-foreground leading-[2.1]">
                  {block.items.map((item, j) => <li key={j}><span className="text-[hsl(var(--gold))]">→ </span>{item}</li>)}
                </ul>
            ...
            <div className="bg-card border-2 border-border rounded-[20px] p-5">
              <div className="text-[9px] tracking-[3px] uppercase text-[hsl(var(--gold))] font-semibold mb-2">Итого в день</div>
              <h3 className="font-display text-xl bg-gradient-to-br from-gold-light to-gold bg-clip-text text-transparent mb-3">~46 минут</h3>
              <ul className="list-none text-[13px] text-muted-foreground leading-[2.1]">
        ...
        {activeTab === 'info' &&
          <div className="px-3.5 pb-24">
            <div className="pt-7 pb-5 text-center">
              <span className="inline-block text-[9px] tracking-[3.5px] uppercase text-[hsl(var(--gold))] border border-[hsl(var(--gold)/0.28)] px-4 py-1 rounded-full mb-3.5 font-medium">Наука & Практика</span>
              <h2 className="font-display text-[28px] font-bold bg-gradient-to-br from-gold-light to-destructive bg-clip-text text-transparent">О курсе</h2>
            </div>
            {infoBlocks.map((block, i) =>
              <div key={i} className={`bg-card border rounded-[20px] p-5 mb-3 ${block.color === 'warning' ? 'border-destructive/20' : 'border-border'}`}>
                <div className={`text-[9px] tracking-[3px] uppercase font-semibold mb-2 ${block.color === 'warning' ? 'text-destructive' : block.color === 'neck' ? 'text-destructive' : 'text-[hsl(160,50%,45%)]'}`}>{block.label}</div>
                <h3 className="font-display text-lg mb-3 text-foreground">{block.title}</h3>
                <p className="text-[13px] text-muted-foreground leading-[1.85] [&_strong]:text-[hsl(var(--gold))]" dangerouslySetInnerHTML={{ __html: block.text }} />
              </div>
            )}
            <div className="bg-card border border-border rounded-[20px] p-5 mb-3 mt-2">
              <div className="text-[9px] tracking-[3px] uppercase text-[hsl(var(--gold))] font-semibold mb-3">Режим отображения</div>
              <div className="flex gap-2 mb-3">
                {([['auto', '🤖 Авто'], ['full', '✨ Полный'], ['lite', '⚡ Экономный']] as [PerformancePreference, string][]).map(([p, label]) => (
                  <button key={p} onClick={() => setPreference(p)} className={`flex-1 py-2 px-1 rounded-xl text-[12px] font-semibold font-body border transition-all ${preference === p ? 'bg-[hsl(var(--gold))] text-[#1C1208] border-[hsl(var(--gold))]' : 'bg-card/50 border-border text-muted-foreground'}`}>{label}</button>
                ))}
              </div>
              <p className="text-[11.5px] text-muted-foreground leading-relaxed">
                {preference === 'auto' ? `Авто: ${resolvedTier === 'full' ? 'FULL' : 'LITE'} (счёт ${debugInfo.score})` : preference === 'full' ? 'Принудительно: полный режим' : 'Принудительно: экономный режим'}
              </p>
              <details className="mt-2">
                <summary className="text-[11px] text-muted-foreground/60 cursor-pointer select-none">Подробности детекции</summary>
